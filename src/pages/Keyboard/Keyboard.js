...
export default function Keyboard() {
  ...
  useEffect(() => {
    ...
    const tasksRef = ref(db, `/Keyboard/${Id}`);
  ...
  const Set1 = () => {
    console.log('sss');
    writeUserData1(Led, Type);
  };
  function writeUserData1(led, type) {
    const db = getDatabase();
    const a = Math.floor(Math.random() * 100);
    set(ref(db, `/Keyboard/${Id}`), {
      Led: led || document.getElementById('Led').defaultValue,
    ...
    <div className="newUser">
      ...
      <form className="newUserForm">
        ...
        <div className="newUserItem">
          ...
          <input defaultValue={data.Led} onChange={Update11} type="text" id="Led" />
        </div>
        <div className="newUserItem">
          <label htmlFor="Type">Type</label>
          <input defaultValue={data.Type} onChange={Update22} type="text" id="Type" />
        </div>
        <div className="newUserItem">
